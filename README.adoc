...
Starting with the Eureka-enabled "hello world" example from http://TODO/gs-service-registry[Registering and Discovering Services], you'll start by adding a circuit breaker to the client code.
...
Here the application is given the name "greetingclient" in Cloud Foundry, the same name used in http://TODO/gs-service-registry[Registering and Discovering Services]. You are welcome to name it whatever you wish, but remember the name, because you'll need it when you bind the application to the Circuit Breaker Dashboard service. 
